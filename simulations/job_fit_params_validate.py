...
EXFIELD = base.load_extfield().data.resample(time="1D").sum()
...
def simulate(pars: dict[str, float], peaks: pd.DatetimeIndex):
    ef = base.load_extfield()

    base.log(f"Using:: α={pars['alpha']}, β={pars['beta']}, γ={pars['gamma']}")
    ...
    result = pd.DataFrame(pd.concat(cascades))
...
base.log("Loading Graphs")
days = base.days()
print(len(days))
GRAPH_ADJ = base.load_graph(full=False, days=days).drop_duplicates()
GRAPH_TMP = base.load_graph(full=True, days=days)
base.log("Loaded Graphs")

# %%

# def main():
# """Do the main."""
REAL_DELAY = base.load_real_delay().fillna(0.0)
baseline = REAL_DELAY.median().sort_index()
print(baseline)

delay = (REAL_DELAY - baseline).clip(lower=0).loc[days]
print(delay)
...
delays_full = simulate(pars, days)

# %%

delays_full["time"] = pd.DatetimeIndex(delays_full["time"])
delays_full = (
    delays_full.drop(columns=["failing"])
    .set_index(["node", "time"])
    .groupby(["node", pd.Grouper(freq="1D", level=1)])
    .sum()
    .reset_index()
    .pivot(columns="node", index="time", values="value")
    .fillna(0.0)
)
delays_full = delays_full.reindex(days, fill_value=0.0)
delays = delays_full.sum(1).to_frame(name="value")

delays["rain"] = base.load_rain(True).sum(1).resample("1D").sum() * 1000
delays["rain"] = base.load_rain(False, "mean").resample("1D").sum() * 1000

delays["real"] = delay.sum(axis=1).to_numpy()
delays = delays.loc[days]
print(delays)
...
def plot_all_days(results: pd.DataFrame):
    ...
    # results = pd.concat([results, base.load_rain().resample("1D").sum()])
    # Lineplots
    ...
    ax_rep.fill_between(
        results.index, results["real"] / 60, color="C8", label="Reported excess", lw=1, alpha=0.8
    )
    ax_rep.plot(
        results.index,
        results["value"] / 60,
    ...
    print("---")
    print(results["rain"].quantile([0.05, 0.5, 0.95]))
    print("---")
    add_backgroundgrad(
        ax_rain,
    ...
    corr = stats.spearmanr(
        results[results["rain"] > 3]["value"], results[results["rain"] > 3]["real"]
    )
    scttr = ax_scat.scatter(
        results["value"] / 60,
        results["real"] / 60,
        c=results["rain"],
        s=results["rain"] * 20 + 10,
        alpha=0.5,
        cmap="RdBu",
        lw=0.1,
        edgecolors="k",
    ...
    l1 = ax_scat.legend(
        handles=[
            lines.Line2D(
                ...
                [0],
                marker="o",
                markersize=np.sqrt(x * 20 + 10),
                lw=0,
                color=scttr.cmap(x / VMAX),
                label=f"{x} mm",
                alpha=0.5,
                mew=0.1,
                mec="k",
            )
            for x in range(VMIN, VMAX + 1, 6)
        ],
        fontsize="x-small",
        ncols=1,
        handletextpad=0,
    ...
    text = [
        ...
        if p["rain"] > 5 and p["value"] > 800 * 60
    ]
    adjust_text(
        text,
        objects=scttr,
    ...
    # ax_scat.set_autoscale_on(False)
    xlim = ax_scat.get_xlim()
    ax_scat.plot(np.linspace(*xlim, 20), np.linspace(*xlim, 20), "k-.", alpha=0.5)
    base.add_axis_label(ax_rep, "c")
    base.add_axis_label(ax_scat, "d ")
    ax_rep.grid(False)
...
def plot_multi_days(isodays: list[str]):
    ...
    axs = fig.subplots(
        nrows=len(isodays),
        gridspec_kw={"left": 0.67, "right": 0.99, "top": 0.95, "bottom": 0.1, "hspace": 0.07},
        sharex=True,
        sharey=True,
    )
...
def _plot_one_day(isoday: str, ax_real, ax_pred, ax_stress, ax_scat):
    ...
    kwargs = dict(
        rasterized=False, cmap="RdBu", vmin=0.0, vmax=0.5, lw=0.0, edgecolor="k", alpha=0.5
    )
    day = geopd.GeoDataFrame(
        day.sort_values("rain"), geometry=GRAPH_ADJ.nodes().loc[day.index, "geometry"]
    )
    print(day["rain"].quantile([0, 0.1, 0.5, 0.9, 1]))

    raindata.plot.pcolormesh(
        ax=ax_stress, add_colorbar=False, lw=0, rasterized=True, vmax=0.5, cmap="Blues"
    )
    ...
    ax_pred.scatter(
        day["geometry"].x,
        day["geometry"].y,
        s=lin_map(day["value"], (0, 100), (0, 30)) if day["value"].max() > 0 else 0,
        c=day["rain"],
        **kwargs,
    )
    ax_pred.set(xlim=(100, 135))
    ...
    spear = stats.spearmanr(day["value"][day["value"] > 0], day["real"][day["value"] > 0])
    ax_scat.annotate(
    ...
    for ax in [ax_pred, ax_real, ax_stress]:
        GRAPH_ADJ.edges().plot(
            ax=ax, lw=0.1, color="#999999", alpha=0.5, zorder=10, rasterized=True
        )

    text = [
        ax_scat.text(d["value"], d["real"], base.shorten_name(str(node)), fontsize="small")
        for node, d in day.iterrows()
        if d["value"] > day["value"].quantile(0.998) or d["real"] > day["real"].quantile(0.998)
    ]
    ax_scat.set(xscale="symlog", yscale="symlog", aspect=1, xlim=(-0.4, 2e5), ylim=(-0.4, 9e4))
    adjust_text(
        text,
...
def plot_one_day(day: pd.DataFrame, raindata: xarray.DataArray, title: str = "One day"):
    ...
    ax1.set(title=title, ylabel="Reported excess delay (mins)", xlabel="Predicted delay (mins)")
...
def main() -> None:
    ...
    for isoday in ["2020-01-06", "2019-12-29", "2020-01-26", "2020-01-25"]:
...
def _get_one_day(isoday: str) -> tuple[pd.DataFrame, xarray.DataArray]:
    print(isoday)
    try:
        daily_d = delays_full.loc[isoday]
    except KeyError:
        print("Nokey")
        daily_d = pd.DataFrame([], columns=["value"])
    raindata = EXFIELD.sel(time=isoday)
    ...
    assert "rain" not in daily_d.index
    assert "rain" not in delay.loc[isoday]
    assert "rain" not in base.load_rain(True).resample("1D").sum().loc[isoday]

    return pd.DataFrame(
        {
            "value": daily_d,
            "real": delay.loc[isoday],
            "rain": base.load_rain(True).resample("1D").sum().loc[isoday],
        }
    ).fillna(0), raindata
