...
def sim_all(
    ...
    if fname is not None:
        pd.DataFrame(delay_evolution, columns=("hours", "mean_delay", "max_delay")).to_csv(
            fname, index=False
        )
    ...
    for _ in trange(100000, dynamic_ncols=True, leave=False):
        state = mod.state.copy()
...
def main() -> None:
    ...
    # build the stressor
    stressor = exfield_like(base.load_extfield().get(day="2020-01-01"), fill_values=0.001)

    # Params
    params = base.params()
    base.log(params)
    print(base.params(df="ci"))
...
def find_bounds() -> tuple[float, float]:
    """find lower and upper bounds to the region where steady state converge."""
    base.log("Computing steady state bounds.")
    stressor = exfield_like(base.load_extfield().get(day="2020-01-01"), fill_values=1)
    rain = graph_adj.integrate(stressor, trange=pd.Timestamp("2020-01-01 08:00:00"), ds=1.0)
    ...
    base.log(f"Lower bound: {lbound}")
    base.log(f"Upper bound: {ubound}")
    ...
    base.log(f"bound: {res['low']}")
    base.log(f"bound: {res['high']}")

    with open("./stats/ext_field_steady_state_bounds.json", "wt") as fout:
        json.dump(res, fout)
...
def find_critical_value():
    ...
    res = []
    for s in np.linspace(0, 0.003, 151):
        _res = {"stressor": s}
