...
def _plot_geo_perturbation(
    ...
    base.log(stats.spearmanr(measure[name].fillna(0), measure["node_degree"]))
    base.log(stats.spearmanr(measure[name].fillna(0), measure["node_betweenness"]))
    ...
    measure.h3.geo_to_h3_aggregate(3, operation="sum").plot(
        column=name, ax=ax_aggr, cmap="Reds", aspect=None
    )

    # Prepare basemap
    for ax in (ax_aggr, ax_map):
        GRAPH.edges().plot(ax=ax, lw=0.1, color="C1", alpha=0.2, rasterized=True, aspect=None)
...
def load_data(degree: bool | None = None, betweenness: bool | None = None) -> geopd.GeoDataFrame:
    base.log("Loading data")
    coverage = pd.read_csv(base.CACHE / "coverage_data.csv.gz", index_col=["time", "station"])
    coverage = coverage.loc["2020-01-01T00:00:00"]
    base.log("Converting to graph")
    graph = GRAPH
    base.log("To networkx")
