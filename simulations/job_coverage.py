...
PERTURBATION_STATE = 120  # minutes
PERTURBATION_STRESSOR = 0.1  # 0.08m/h rain
...
def sim_station(model: Diffusion, station: str, kind: str) -> pd.DataFrame:
    # save initial state
    # model.conclude_step(model.ex_field.trange()[0].to_datetime64(), threshold=0)

    for _, lef in enumerate(model.ex_field.extreme_events("simple")):
        hour = lef.trange()[1]
    ...
    if len(delays) == 0:
        return pd.DataFrame(
            {kind: [], kind + "_count": [], kind + "_risk": [], "time": [], "station": []}
    ...
    delays_cumul = (
        delays.groupby("time").sum().stack().rename_axis(["time", "station"]).rename(index=kind)
    )
    delays_count = (
        delays.groupby("time")
        .count()
...
def prepare_initial_state(
    ...
    if kind == "state":
        ...
        gg = loc_graph.subset([("hour", 9), ("weekday", False), ("month", False)]).drop_duplicates()
        # Sum the rain along all incoming links
        integral = gg.to_matrix(weight=rain).multiply(gg.to_matrix(weight="count")).sum(1).A.ravel()
...
def sim_all(data):
    ...
    prepare_initial_state(
        "stressor", mod, station, graph_tmp, rain.loc[[station in link for link in rain.index]]
    )
...
def main() -> None:
    ...
    stressor = exfield_like(
        base.load_extfield().get(day="2020-01-01"), fill_values=PERTURBATION_STRESSOR
    )
